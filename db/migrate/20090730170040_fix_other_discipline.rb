...
class FixOtherDiscipline < ActiveRecord::Migration
      ...
      if csv_event.nil?
        ...
        csv_event.event_types.each do |et|
          if !db_event.event_types.include?(et)
            puts "#{db_event.dn} ... add event type '#{et.discipline_id}:#{et.name}'"
  ...
  def self.load_events_from_tab_delimited_file(csv_data)
    ...
    CSV.open(csv_data, 'r', "\t") do |row|
      ...
      if discipline.nil? 
        puts "[warning] nil discipline '#{row[COL_DISCIPLINE].to_s.capitalize}' for #{ev.dn}"
      else
